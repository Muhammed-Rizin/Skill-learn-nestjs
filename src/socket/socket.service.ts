...
export class ChatService {
  ...
  async getChats(userId: string, @Res() res: Response) {
    try {
      const chatDetails = await this.messageModel
        .find({ $or: [{ 'messages.sender': userId }, { users: userId }] })
        .populate('messages.sender')
        .populate('messages.receiver');
  ...
  async getChatHistory(
    ...
    try {
      ...
      const chatData = await this.messageModel
      ...
      const start =
        total - ((skip + limit) as number) > 0
          ? total - ((skip + limit) as number)
          : 0;

...
export class ScheduleService {
    ...
    async getInprogress(id : string, page : number, @Res() res : Response) {
        try {
            ...
            const total = (await this._scheduleSchema.find({from : id, time :{$gte : now}}).populate('to')).length
    ...
    async getCompleted(id : string, page : number, @Res() res : Response) {
        try {
            ...
            .sort({time : -1}).skip(skip).limit(limit)
            const total = (await this._scheduleSchema.find({from : id, time :{$lte : now}}).populate('to')).length
    ...
    async getInprogressOfUser(id : string, page: number, @Res() res : Response) {
        try {
            ...
            .sort({time : 1}).skip(skip).limit(limit)
            const total = (await this._scheduleSchema.find({to : id, time :{$gte : now}}).populate('from')).length
            return res.status(200).json({data, total})
        } catch (error) {
            console.log(error.message)
            res.status(500).json({status: 'error', message: 'internal server error'}) 
    ...
    async getCompletedOfUser(id : string, page : number, @Res() res : Response) {
        try {
            ...
            .sort({time : -1}).skip(skip).limit(limit)
            const total = (await this._scheduleSchema.find({to : id, time :{$lte : now}}).populate('from')).length
            return res.status(200).json({data, total})
        } catch (error) {
            console.log(error.message)
            res.status(500).json({status: 'error', message: 'internal server error'}) 

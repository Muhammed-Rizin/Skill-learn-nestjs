...
export class NotificationService {
    constructor(
        @InjectModel('Notification') private readonly notificationModel : Model<NotificationType>,
    ){}
    ...
    async updateStatus( userId : string, roomId : string, res : Response) {
        try {
            await this.notificationModel.updateMany({to : userId, roomId : roomId}, {$set : {read : true}})
            return res.status(200).json({message : 'success'})
        } catch (error) {
            return res.status(500).json({message : 'internal server error'})
        }

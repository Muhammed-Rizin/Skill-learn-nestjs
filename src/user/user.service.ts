...
export class UserService {
  ...
  async userRegister(data: User, @Res() res: Response) {
    try {
      ...
      res
        .status(500)
        .json({ status: 'error', message: 'User register failed' });
    }
  }
  ...
  async userForgotPassword(email: string, @Res() res: Response) {
    try {
      ...
      const sendMail = this.forgetPassword.forgetPassword(
        userData.firstName,
        email,
        token,
      );
  ...
  async sendVerifyEmail(userId: string, @Res() res: Response) {
    try {
      ...
      const sendMail = await this.verification.userVerifyEmail(
        userData.firstName,
        userData.email,
        token,
      );
